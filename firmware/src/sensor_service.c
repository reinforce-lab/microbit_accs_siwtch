...
static void onRWAuthReq(sensor_service_t *p_context, ble_evt_t *p_ble_evt)
{
    ...
    if(p_auth_req->type == BLE_GATTS_AUTHORIZE_TYPE_READ) {
        if( p_auth_req->request.read.handle == p_context->sensor_setting_char_handle.value_handle){
            ...
            getSensorThreshold(&data);
            length = serializeSensorThreshold(buffer, &data);
        } else {
            // 一致しないハンドラ
            return;
        }
        ...
        if( p_auth_req->request.write.handle == p_context->sensor_setting_char_handle.value_handle){
            // 閾値データを書き込む。
            SensorThreshold data;
            bool result = deSerializeSensorThreshold(&data, p_auth_req->request.write.data);
            if(result) {
                setSensorThreshold(&data);
            }
        } else {
            // 一致しないハンドラ
            return;
...
static void onWrite(sensor_service_t *p_context, ble_evt_t * p_ble_evt)
{
    ...
    if(p_evt_write->len == 2) {
        if(p_evt_write->handle == p_context->sensor_data_char_handle.cccd_handle) {
            p_context->is_sensor_data_notifying = ble_srv_is_notification_enabled(p_evt_write->data);
            return;
        }
    }
}
...
static void addService(sensor_service_t *p_context, uint8_t uuid_type)
{
    ...
    // リアルタイムデータ
    params.uuid              = SENSOR_DATA_CHAR_UUID;
    ...
    err_code = characteristic_add(p_context->service_handle, &params, &(p_context->sensor_data_char_handle));
    APP_ERROR_CHECK(err_code);
    
    // セッティング
    params.uuid              = SENSOR_SETTING_CHAR_UUID;
    params.max_len           = 5;
...
void sensorService_handleBLEEvent(sensor_service_t *p_context, ble_evt_t * p_ble_evt)
{
    switch (p_ble_evt->header.evt_id) {
            ...
            p_context->connection_handle = BLE_CONN_HANDLE_INVALID;
            p_context->is_sensor_data_notifying = false;
            break;
...
{
    if( ! p_context->is_sensor_data_notifying) {
        return false;
    }
    ...
    hvx_params.handle = p_context->sensor_data_char_handle.value_handle;
