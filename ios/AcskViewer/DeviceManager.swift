...
open class DeviceManager : NSObject, CBCentralManagerDelegate
{
    ...
    // Properties, KVO
    @objc dynamic open fileprivate(set) var devices:[Device] = []
    ...
    open static let sharedInstance: DeviceManager = DeviceManager()
    
    fileprivate override init() {
        queue = DispatchQueue(label: "sensor.ble-queue") // serial queue
    ...
    open func scan(_ duration:TimeInterval = 5.0, callback:((_ remaining: TimeInterval) -> Void)?)
    {
        ...
        // 接続済のペリフェラルを取得する
//        for peripheral in (manager!.retrieveConnectedPeripherals(withServices: [DeviceUUIDs.advertisingServiceUUID])) {
        for peripheral in (manager!.retrieveConnectedPeripherals(withServices: [DeviceUUIDs.SensorServiceUUID])) {
            addPeripheral(peripheral, name:nil)
        }
        
        // スキャンを開始する。
//        manager!.scanForPeripherals(withServices: [DeviceUUIDs.advertisingServiceUUID], options: nil)
        manager!.scanForPeripherals(withServices: [DeviceUUIDs.SensorServiceUUID], options: nil)
        isScanning = true
    ...
    func addPeripheral(_ peripheral: CBPeripheral, name: String?)
    {
        ...
        if !devices.contains(where: { element -> Bool in element.peripheral == peripheral }) {
            var devs = Array<Device>(self.devices)
            devs.append(Device(manager: self.manager!, peripheral: peripheral, name: name))
            DispatchQueue.main.async(execute: {
                self.devices = devs
            })
        }
