...
open class Device : NSObject, CBPeripheralDelegate
{
    // MARK: variables
    open unowned let manager: CBCentralManager
    open let peripheral:      CBPeripheral

    open weak var delegate: DeviceDelegate?
    ...
    open fileprivate(set) var sensorService: SensorSerivce?
    ...
    {
        // サービスの発見
        let serviceUUIDs = Array(DeviceUUIDs.ServiceUUIDs.keys)
    ...
    open func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?)
    {
        ...
        for service in peripheral.services! {
            let characteristicsUUIDs = DeviceUUIDs.ServiceUUIDs[service.uuid]
...
//   debugPrint("\(#function), \(service.UUID).")
        ...
        switch service.uuid {
        case DeviceUUIDs.DeviceInformationServiceUUID:
            self.deviceInformationService = DeviceInformationService(device:self)
        case DeviceUUIDs.BatteryServiceUUID:
            self.batteryLevelService = BatteryService(device:self)
        case DeviceUUIDs.SensorServiceUUID:
            self.sensorService = SensorSerivce(device: self)
...
//debugPrint("didUpdate: \(characteristic.uuid) \(data)")
        ...
        case DeviceUUIDs.SensorServiceUUID:
            self.sensorService?.didUpdateValue(characteristic, data: data)
